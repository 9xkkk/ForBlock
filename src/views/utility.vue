        ...
        <el-card class="chart-card">
          <el-tabs v-model="modelCmp" class="" @tab-click="handleTabClick">
            <el-tab-pane label="模型对比" name="modelCmp">
              <BarChart ref="chartRef" :chartData="chartData" :chartOptions="chartOptions" />
            </el-tab-pane>
...
import BarChart from 'D:/CODE/ForBlock/src/components/BarChart.vue';
...
const updateChart = (accuracyData1, accuracyData2) => {
  ...
  if (chartRef.value && chartRef.value.chart) {
    console.log('Chart instance:', chartRef.value.chart);
    chartRef.value.chart.update();
  } else {
    console.error('Chart reference is not set or update method is missing.');
  }
};
